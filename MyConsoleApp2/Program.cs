...
class Program
{
    ...
    static void AddProduct(List<Product> products, int maxCount)
    {
        ...
        if (mode == "2")
        {
            ...
            try
            {
                int constructorChoice = rnd.Next(1, 5);
                Product auto;
                ...
                switch (constructorChoice)
                {
                    case 1:
                        ...
                        break;
                    case 2:
                        auto = new Product(names[rnd.Next(names.Length)], rnd.Next(100, 2000));
                        Console.WriteLine("✅ Використано конструктор з двома параметрами (назва, ціна).");
                        break;
                    case 3:
                        auto = new Product(names[rnd.Next(names.Length)], rnd.Next(100, 2000), rnd.Next(1, 50));
                        Console.WriteLine("✅ Використано конструктор з трьома параметрами (назва, ціна, кількість).");
                        break;
                    default:
                        auto = new Product(
                        ...
                        Console.WriteLine("✅ Використано конструктор з усіма параметрами.");
                        break;
                }
    ...
    static void Demonstrate(List<Product> products)
    {
        ...
        if (int.TryParse(Console.ReadLine(), out int num) && num >= 1 && num <= products.Count)
        {
            ...
            while (true)
            {
                ...
                Console.WriteLine("2 - ShowInfo(bool)");
                Console.WriteLine("3 - ShowInfo(string)");
                Console.WriteLine("4 - GetTotalValue()");
                Console.WriteLine("5 - IsExpired()");
                Console.WriteLine("0 - Повернутися");
                Console.Write("Ваш вибір: ");
                string sub = Console.ReadLine();

                if (sub == "1") p.ShowInfo();
                else if (sub == "2") p.ShowInfo(true);
                else if (sub == "3") p.ShowInfo("Додаткова інформація про товар:");
                else if (sub == "4") Console.WriteLine($"Загальна вартість: {p.GetTotalValue()} грн");
                else if (sub == "5") Console.WriteLine(p.IsExpired() ? "Товар прострочений" : "Товар не прострочений");
                else if (sub == "0") break;
