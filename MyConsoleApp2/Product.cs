...
public class Product
{
    ...
    public Product(string name, double price)
        : this(name, price, 1, DateTime.Now, Category.Other)
    { }

    public Product(string name, double price, int quantity)
        : this(name, price, quantity, DateTime.Now, Category.Other)
    { }
    ...
    public void ShowInfo(bool shortInfo)
    {
        if (shortInfo)
            Console.WriteLine($"{Name} — {Price} грн, {Quantity} шт.");
        else
            Console.WriteLine(GetInfoString());
    }
